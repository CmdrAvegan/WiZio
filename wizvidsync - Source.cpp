...
void SavePresetToFile(const Preset& preset) {
    ...
    // Save preset details
    presetJson["name"] = preset.name.toStdString();
    presetJson["darknessThreshold"] = darknessThreshold.load();
    presetJson["colorBoostIntensity"] = colorBoostIntensity.load();
    presetJson["darknessThresholdValue"] = darknessThresholdSlider->value();
    presetJson["dynamicBrightnessEnabled"] = dynamicBrightnessCheckbox->isChecked();
    presetJson["mainImageScale"] = mainImageScale.load();
    presetJson["motionImageScale"] = motionImageScale.load();
    presetJson["patternImageScale"] = patternImageScale.load();
    presetJson["maxBrightness"] = maxBrightness.load();
    presetJson["darknessSliderValue"] = darknessSlider->value();
    presetJson["adjustBrightness"] = adjustBrightness.load();
    presetJson["colorBoostEnabled"] = colorBoostEnabled.load();
    presetJson["ambilightModeEnabled"] = ambilightModeEnabled.load();

    // Save lights
    presetJson["lights"] = nlohmann::json::array();
    for (const Light& light : preset.lights) {
        nlohmann::json lightJson = {
            {"name", light.name.toStdString()},
            {"ipAddress", light.ipAddress.toStdString()}
        };
    ...
    file << presetJson.dump(4); // Pretty-print with 4 spaces
}
...
Preset LoadPresetFromFile(const QString& presetName) {
    ...
    preset.darknessThresholdValue = presetJson["darknessThresholdValue"];
    preset.darknessSliderValue = presetJson["darknessSliderValue"];

    // Load lights
    preset.lights.clear();
    for (const auto& lightJson : presetJson["lights"]) {
        Light light;
        light.name = QString::fromStdString(lightJson["name"]);
...
void SavePreset(const QString& presetName) {
    ...
    preset.dynamicBrightnessEnabled = dynamicBrightnessCheckbox->isChecked();
    ...
    preset.maxBrightness = maxBrightness.load();
    preset.lights = lights;
    preset.darknessAdjustment = darknessSlider->value();
    preset.darknessSliderValue = darknessSlider->value();
    preset.darknessThresholdValue = darknessThresholdSlider->value();
...
void LoadPreset(const QString& presetName) {
    ...
    for (const Light& light : preset.lights) {
        ...
        // Check if the light name contains "RGB (x,x,x)"
        if (light.name.contains("RGB (")) {
            QStringList parts = light.name.split(" - ");
            QString baseName = parts.mid(0, parts.size() - 1).join(" - ");
            QString rgbPart = parts.last().split("RGB (").last().split(")").first();
            ...
            if (rgbValues.size() == 3) {
                QString coloredRGBText = QString("<span style='color: rgb(%1, %2, %3);'>RGB (%1, %2, %3)</span>")
                                          .arg(rgbValues[0])
                                          .arg(rgbValues[1])
                                          .arg(rgbValues[2]);
                label->setText(baseName + " - " + coloredRGBText);
            } else {
                label->setText(light.name);
            }
        } else {
            label->setText(light.name);
        }
    ...
    // Update current preset label
    if (currentPresetLabel) {
        currentPresetLabel->setText("Current Preset: " + preset.name);
    }
